...
module.exports = class Customer {
    ...
    updateById(id,customer, cbResult){
        let queryString = 'Update customer SET email = ?, firstName = ?,  lastName = ?, age= ?, phone = ?, adress = ?, subject = ?';
        queryString += ' WHERE id = ?';
        sql.query(queryString,
            [customer.email, customer.firstName, customer.lastName, customer.age, customer.phone, customer.adress, customer.subject, parseInt(id)],
            (err, result) => {
                if (err){
                    console.log("error: ", err);
                    //err zurückgeben, data = null
