...
function validateFormData(data) {
    ...
    result = validateLib.checkRequired("number", data.phone);
    if (result.isNotValid) { return result; }

    result = validateLib.checkRequired("email", data.email);
    if (result.isNotValid) { return result; }

    result = validateLib.checkRequired("adress", data.adress);
    if (result.isNotValid) { return result; }
    ...
    result = validateLib.checkLength("subject", data.subject, 3, 250);
    if (result.isNotValid) { return result; }

    result = validateLib.checkLength("adress", data.lastName, 3, 100);
    if (result.isNotValid) { return result; }
    ...
    //check number
    result = validateLib.checkPhone("number", data.phone);
    if (result.isNotValid) { return result; }
